...
<body>
              ...
              <tbody class="studentinfo">
                <c:forEach items="${back}" var="item" varStatus="status">  
                  <tr>
                    ...
                    <td>
                        <button  id="${item.borrowid}" class="btn btn-success back ">同意还书</button>
                    </td>  
                  </tr>  
                </c:forEach>  
              </tbody>
    ...
    $(".back").on('click',function(){
        overlay_show();
        var url="/BMS/adminaction/askback";
        var dataMessage = {borrowid :$(this).attr("id")};
        $.post(url,dataMessage,function(data){
            overlay_hide();
            if(data){
                location.href = "http://localhost:8080/BMS/adminaction/viewbookaskback"
            }
        });
    });
    </script>
